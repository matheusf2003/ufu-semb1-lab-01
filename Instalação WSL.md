...
É boa prática manter o sistema operacional atualizado. Para isso, temos que
...
Antes de iniciarmos a instalação das ferramentas necessárias para a configuração
...
### 4.1 Acesso ao sistema de arquivos do Linux no Windows Explorer
...
arquivos Linux de forma gráfica. Para situações como a descrita, podemos
...
O *Windows Subsystem for Linux* permite que desenvolvedores Windows executem
...
Assim, devemos primeiramente verificar qual a versão do kernel está instalada
...
* uma regra de *firewall* para permitir que todas as sub-redes locais se conectem ao
serviço *usbipd*.
...
**seja diferente, a linha de comando deverá ser modificada adequadamente.**
...
Antes de conectar o gravador **ST-LINK** na USB, certifique-se de que o terminal
...
Observe que, ao listar novamente os dispositivos USB, vemos que o estado do
gravador ST-LINK mudou de **Not attached** para **Attached - Ubuntu-20.04**.
...
Os sistemas operacionais baseados em Unix, dos quais o Linux faz parte, têm a
capacidade de definir de forma detalhada os direitos de acesso aos arquivos,
...
conjunto de caracteres ```crw-------```. O primeiro caractere indica a natureza do
...
o **udev** executa um *script* ao receber um evento de algum dispositivo, por
exemplo, ao conectarmos um dispositivo USB.

Os ***scripts*** necessários para ajustar os direitos de acesso ao **ST-LINK**
estão na pasta ***lab-01***. Acesse a pasta do laboratório e copie todos os
